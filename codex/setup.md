...
- **outside-scope 觀察**(留 Lane A):`-c sandbox_permissions=...` 未在 Codex 啟動 banner 反映(banner 仍顯示 `sandbox: workspace-write`),候選 a config key 不適用 / b PowerShell quoting / c CLI 版本漂移,**不擅自 verify**
...
### Lane A:sandbox root cause 追(D1 失敗後候選升級)

D1 實測證明 sandbox 失敗**不限於** Claude Code 嵌套 spawn(獨立 PowerShell 也撞)→ 問題定位範圍暫擴大為「Codex CLI Windows sandbox `CreateProcessAsUserW failed: 5`」。

候選(Wayne 自決時機):
- 查 GitHub issue 找 Codex CLI Windows sandbox `CreateProcessAsUserW` / `disk-full-read-access` 解
- verify outside-scope a/b/c(`-c sandbox_permissions=...` 在 Codex 0.128.0 是否被讀)
- ~~試 Claude Code permission mode~~ → 降級(D1 證明獨立 PowerShell 也撞,permission mode 不解此問題)
- 接受 sandbox 限制 + 文件化失敗模式為主路徑(`/codex:setup` 等 thread 啟動類仍可用)
