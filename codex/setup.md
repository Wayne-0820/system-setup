...
## Stop-time review gate 行為(2026-05-06 實證)

> **Best-effort / silent advisory** — **不可**寫成可靠 stop-time review gate。

2026-05-06 tracked-dirty probe(派工 `assignments/2026-05-06_codex-review-gate-tracked-dirty-probe.md` + progress report `progress-reports/2026-05-06_codex-review-gate-tracked-dirty-probe.md`)實證以下行為:

1. **Staged dirty 觸發 inspection**:repo 根放 minimal staged probe(`A  codex-review-gate-probe.md`,純 ASCII,UTF-8 no BOM)後 session 2 自然 stop,review gate enabled 條件下確實 fire 一輪 inspection。
2. **Sandbox.log 觀察到 inspection 命令**:`Get-ChildItem -Force` / `git status --short --branch` / `git log --oneline --decorate -n 12`(三個 read-only 探查命令,經 `~/.codex/.sandbox/sandbox.log` setup refresh + helper launch trail 落地)。
3. **無 error**:sandbox.log 在 inspection 期間**無** `ERROR` / `failed` / `CreateProcessAsUserW failed` / auth 失敗 entry。
4. **使用者端無 surface**:Claude 對話框**無** review output / blocking prompt / 綠 / 黃 / 紅 finding;Codex Desktop CODEX COMPANION TASK panel **無** 可見 review finding / blocking 結果。
5. **目前結論**:review gate enabled 會在 dirty tree(至少 staged add 形式)觸發 silent inspection,但**未證實**有 user-visible Codex review output 或 blocking gate 機制。
6. **降級語意**:行為定位為 **best-effort / silent advisory**;不能依賴它替代 commit 前手動 review,也不能寫成「stop 必有 review 把關」的可靠 gate。

補強脈絡:`progress-reports/2026-05-05_codex-review-gate-hook-log-probe.md` 已實證 Claude Code 端 user-level + project-level `settings.json` / `settings.local.json` 共 3 份均無 `hooks` 段;review gate 機制不在 Claude Code settings hook 層,觸發點在 plugin internal / Codex Desktop / 其他未驗證層。
...
**最後更新**:2026-05-06
**版本**:1.1(2026-05-06 補 stop-time review gate 行為實證段)
