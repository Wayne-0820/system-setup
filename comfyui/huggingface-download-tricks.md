...
```powershell
Get-ChildItem -Path "D:\Models\diffusion\checkpoints" -Filter "*.safetensors"
```
...
```powershell
cmd /c dir /b "D:\Models\diffusion\checkpoints\*.safetensors"
```

#### 方案 B:不用 -Filter,改用 Where-Object

```powershell
Get-ChildItem "D:\Models\diffusion\checkpoints" | Where-Object { $_.Name -like "*.safetensors" }
```
...
## 命名:HF 通用 placeholder 檔名 → 加 repo 識別前綴

很多 HF repo 的權重檔用 diffusers / transformers 預設的 placeholder 檔名,在 repo 頁面看得出脈絡,**下載到本地後完全沒識別性**:

| HF 原檔名(placeholder) | 出現場景 |
|---|---|
| `unet_fp8.safetensors` | UNET 量化版 repo 常用 |
| `diffusion_pytorch_model.safetensors` | diffusers 標準命名(常見於 LoRA / ControlNet / VAE) |
| `pytorch_model.bin` / `model.safetensors` | transformers 標準命名 |

**規則**:下載後**必須加 repo 識別前綴**改名,通常用 `<repo-name>` 或更精準的 `<repo-name>-<variant>` 格式。

**實例**:

| HF 原始 | 落地檔名 | 來源 repo |
|---|---|---|
| `unet_fp8.safetensors` | `flux.1-fill-dev-OneReward-fp8.safetensors` | `yichengup/flux.1-fill-dev-OneReward` |
| `diffusion_pytorch_model.safetensors` | `Flux-Turbo-Alpha.safetensors` | `alimama-creative/FLUX.1-Turbo-Alpha` |

**例外**:HF 原檔名**已含識別性**(像 `removal_timestep_alpha-2-1740.safetensors`)→ **維持原檔名,不必改**(版本可追溯,改了反而難對源)。

**判斷準則**:檔名脫離 repo 上下文後,三個月後翻硬碟看得懂這是什麼模型嗎?看不懂 → 改;看得懂 → 不改。

---

## 路徑慣例:模型落地分類

下載後依模型類型落到 `D:\Models\diffusion\` 對應子資料夾:

| 模型類型 | 落地路徑 |
|---|---|
| UNET / DiT 主模型 | `D:\Models\diffusion\diffusion_models\` |
| Checkpoint(SD 1.5 / SDXL 整合包) | `D:\Models\diffusion\checkpoints\` |
| LoRA(精修 / 加速 / 風格) | `D:\Models\diffusion\loras\` |
| CLIP / 文字編碼器 | `D:\Models\diffusion\clip\` |
| VAE | `D:\Models\diffusion\vae\` |
| ControlNet | `D:\Models\diffusion\controlnet\` |
| Upscale 模型 | `D:\Models\diffusion\upscale_models\` |
| Embeddings / Textual Inversion | `D:\Models\diffusion\embeddings\` |
| CLIP Vision | `D:\Models\diffusion\clip_vision\` |

**例外:LLM 類模型**(JoyCaption / Llama)**寫死**在 `D:\Work\ComfyUI_portable\ComfyUI_windows_portable\ComfyUI\models\LLavacheckpoints\`,**不能搬到 `D:\Models\diffusion\`**(節點 hardcode 路徑,不吃 `extra_model_paths.yaml`)。詳見 `comfyui/setup.md`。

**驗證步驟**:下完模型後,啟動 ComfyUI 並用 HTTP API 拉 dropdown:

```powershell
Invoke-RestMethod -Uri "http://127.0.0.1:8188/object_info" |
    ConvertTo-Json -Depth 10 |
    Select-String -Pattern "你的檔名"
```

確認對應節點(UNETLoader / LoraLoaderModelOnly / VAELoader 等)能看到該檔。看不到 → 路徑放錯,或 `extra_model_paths.yaml` 沒指到該子資料夾。

(此招實證有效:2026-04-29 `sd → diffusion` rename 任務煙測就是用 `/object_info` 直接驗 yaml 解析端,比 GUI 載入 workflow 更直接。)
