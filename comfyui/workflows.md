...
> 已建立的 8 個 workflow 完整描述 + 設計理由 + 使用場景。
>
...
| 名稱 | 模型 | 步數 | 速度 | VRAM | 用途 |
|---|---|---|---|---|---|
| `Wan2.2_A14B_T2V-I2V合一_720P_81幀_4步.json` | Wan 2.2 A14B fp8_scaled + Lightx2v 4-step LoRA × 4 | 4(HIGH 2 + LOW 2)| 12.91 分鐘 | 24,070 MB | T2V/I2V 合一 720P/81幀(~3.4 秒)影片生成 |
...
### 11. Wan2.2_A14B_T2V-I2V合一_720P_81幀_4步

**檔案**:`Wan2.2_A14B_T2V-I2V合一_720P_81幀_4步.json`

**用途**:**Wan 2.2 A14B 影片生成主力 workflow**。720P / 81 frames(~3.4 秒影片)生成,T2V / I2V 合一 — bypass 切換模式。

**模型組合**:
- UNET HIGH:`Wan2_2-T2V-A14B_HIGH_fp8_e4m3fn_scaled_KJ.safetensors`(14.0 GB)
- UNET LOW:`Wan2_2-T2V-A14B-LOW_fp8_e4m3fn_scaled_KJ.safetensors`(14.0 GB)
- I2V HIGH:`Wan2_2-I2V-A14B-HIGH_fp8_e4m3fn_scaled_KJ.safetensors`(14.0 GB)
- I2V LOW:`Wan2_2-I2V-A14B-LOW_fp8_e4m3fn_scaled_KJ.safetensors`(14.0 GB)
- Text Encoder:`umt5-xxl-enc-fp8_e4m3fn.safetensors`(6.27 GB,落 `clip\`)
- VAE:`Wan2_1_VAE_bf16.safetensors`(0.24 GB)
- LoRA × 4(Lightx2v 4-step Lightning):
  - T2V HIGH:`wan22-lightning\Wan22_A14B_T2V_HIGH_Lightning_4steps_lora_250928_rank128_fp16.safetensors`(1.227 GB,2025-09-28 新版 rank128)
  - T2V LOW:`wan22-lightning\Wan22_A14B_T2V_LOW_Lightning_4steps_lora_250928_rank64_fp16.safetensors`(0.614 GB,rank64)
  - I2V HIGH:`wan22-lightning\Wan2.2-Lightning_I2V-A14B-4steps-lora_HIGH_fp16.safetensors`(0.614 GB,/old/ 版本)
  - I2V LOW:`wan22-lightning\Wan2.2-Lightning_I2V-A14B-4steps-lora_LOW_fp16.safetensors`(0.614 GB,/old/ 版本)
- Custom node pack:`ComfyUI-WanVideoWrapper`(Kijai)

**節點結構**:
- WanVideoModelLoader × 2(HIGH + LOW,fp8_e4m3fn_scaled, attention_mode=sageattn)
- WanVideoLoraSelect × 2(LoRA inject 進對應 HIGH/LOW model)
- WanVideoTextEncode(umt5_xxl)
- WanVideoEmptyEmbeds(T2V 模式)/ WanVideoImageToVideoEncode(I2V 模式,bypass 切換)
- WanVideoSampler × 2(HIGH steps 0→2 / LOW steps 2→4)
- WanVideoVAELoader / WanVideoDecode
- SaveAnimatedWEBP

**參數**:
- Total Steps:**4**(HIGH 2 + LOW 2,蒸餾)
- CFG:**1.0**(Lightx2v 蒸餾模型不用 CFG)
- Sampler:euler
- Resolution:1280×720
- Frames:81(~3.4 秒 @ 24fps)
- attention_mode:sageattn(不要加 torch.compile,詳踩坑 #12)
...
- 不要加 `WanVideoTorchCompileSettings` — inductor 在當前 fp8 + sageattn + Blackwell sm_120 環境蓋掉 sageattn fused kernel,反而拖慢 2.5×(詳 setup.md 踩坑 #12)
- 加 `WanVideoBlockSwap` 也會拖慢(CPU offload 在 24GB 邊緣不划算)
...
| 3c | Wan 2.2 T2V/I2V 合一 720P/81幀 4 步 | TBD | Wan 2.2 A14B + Lightx2v 4-step Lightning LoRA(T2V 250928 / I2V /old/)| ✅ 2026-04-30(T2V 煙測 12.91 min,I2V 待煙測)|
