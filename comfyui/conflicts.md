...
| 項目 | 狀態 |
|---|---|
| 已記錄 pack 數 | 6(KJNodes、rgthree-comfy、comfyui_controlnet_aux、ComfyUI_Custom_Nodes_AlekPet、Comfyui-QwenEditUtils、ComfyUI-WanVideoWrapper) |
| 反向索引節點數 | 4(active 衝突)+ 57(同 pack 群衝突,待安裝才升級) |
| 高風險未解衝突 | 0 |
| 中風險待留意 | 4(`AudioConcatenate` / `WanVideoNAG` / `was-node-suite-comfyui` archived 替代節點對照 / `Text Multiline` archived 替代用 `PrimitiveStringMultiline`) |
...
| comfyui_controlnet_aux | repo HEAD | 2026-04-29 | 0(結構性) | [`conflicts-controlnet-aux.md`](./conflicts-controlnet-aux.md) | ✅ active |
| ComfyUI_Custom_Nodes_AlekPet | (架構前已裝) | < 2026-04-29(catalog 04-29) | 0(結構性) | [`conflicts-alekpet.md`](./conflicts-alekpet.md) | ✅ active(部分 sub-node disabled) |
| Comfyui-QwenEditUtils | repo HEAD | 2026-04-29 | 0(結構性) | [`conflicts-qweneditutils.md`](./conflicts-qweneditutils.md) | ✅ active |
| ComfyUI-WanVideoWrapper | repo HEAD | 2026-04-30 | 0(結構性) | [`conflicts-wanvideowrapper.md`](./conflicts-wanvideowrapper.md) | ✅ active |
...
| `Text Multiline` | `was-node-suite-comfyui`(archived) | `PrimitiveStringMultiline` | comfy-core(`comfy_extras.nodes_primitive`) | `Qwen_Edit_2509_保留人物改姿勢_8步.json` | 純 STRING multiline widget → STRING output passthrough。a 替代 = comfy-core 內建,1 input(`value`)/ 1 output(`STRING`)行為完全對等。WAS 原節點 widget 名 `text`,內建 widget 名 `value`,patch JSON 時 `inputs[0].name` + `inputs[0].widget.name` 同步改。Wayne 拍 a 不選 KJNodes `StringConstantMultiline`(b)是因為 b 多 `strip_newlines=true` 預設行為差異(本輪 prompt 單行差異不顯著但保守選 a)|
...
| 2026-04-29 | comfyui_controlnet_aux 安裝(裝前 scoping)| 直接裝,結構性 0 衝突 | 64 個節點全部前綴獨佔(`AIO_*` / `*Preprocessor` / `DepthAnything*` 等),跟既有 4 條 active 反向索引 + KJNodes 全清單 0 重疊。Workflow #3b `AIO_Preprocessor` 必需 |
| 2026-04-29 | ComfyUI_Custom_Nodes_AlekPet catch-up(架構前已裝)| 維持 active,catch-up 紀錄已產 | 16 個前綴 `Argos*` / `DeepTranslator*` / `Google*` / `*PainterNode` 節點,結構性 0 衝突。**ArgosTranslate sub-node 因 cp950 install fail disabled,不修(無 offline 翻譯需求)** |
| 2026-04-29 | Comfyui-QwenEditUtils 安裝(裝前 scoping)| 直接裝,結構性 0 衝突 | 14 個節點全部前綴獨佔(7 個 `*_lrzjason` 後綴 + `QwenEdit*` / `CropWithPadInfo` / `LoadImageReturnFilename`),跟既有 4 條 active 反向索引 + KJNodes 全清單 0 重疊。Workflow #3a-v2 `TextEncodeQwenImageEditPlus_lrzjason` 原派工指定但 C 方案實機跑通走內建路徑(QwenEditUtils 在 #3a-v2 沒被 KSampler 真正消費,留作未來 #5 / #6 Qwen 系列 workflow 候選)|
| 2026-04-29 | `Text Multiline`(WAS archived)vs `PrimitiveStringMultiline`(comfy-core 內建)| comfy-core 內建勝出,was-node-suite-comfyui **仍不裝** | 同 `Mask Fill Holes` 同質決策(was archived 不為單一節點裝整 pack);`PrimitiveStringMultiline` 是 ComfyUI 0.3.x 內建純文字 passthrough,跟 `Text Multiline` 行為完全等價,Workflow #3a-v2 實機跑通驗證 |
| 2026-04-30 | ComfyUI-WanVideoWrapper 安裝(裝前 scoping) | 直接裝,結構性 0 衝突;opencv 補修走 force-reinstall | 142 節點無前綴重疊;`WanVideoNAG` vs KJNodes `WanVideoNAG` class name 不同(Wrapper 是 `WanVideoApplyNAG`)不構成 override;opencv 衝突按教訓 #8 處理 |
...
- [`conflicts-wanvideowrapper.md`](./conflicts-wanvideowrapper.md) — WanVideoWrapper 衝突明細 + pip_overrides 設定
