...
2026-04-28 環境盤點時發現 `D:\Work\ComfyUI_portable\ComfyUI_windows_portable\ComfyUI\custom_nodes\` 實體有 `rgthree-comfy\`,但 `setup.md` 的「已裝 Custom Nodes」清單沒記錄,屬於**架構建立前已存在、漏網沒記錄的 pack**。

按 `setup.md`「裝新 custom node 流程」原本是裝前→比對→裝→產檔。但 rgthree 已在跑,只能反向走:
...
- [`setup.md`](./setup.md) — ComfyUI 配置全貌(本 pack 補入「已裝 Custom Nodes」清單由主窗口處理)
- [`conflicts-kjnodes.md`](./conflicts-kjnodes.md) — KJNodes 衝突明細(per-pack template 來源)
- [`workflows.md`](./workflows.md) — Workflow 清單
