...
當 commit / snapshot / 規則文件變更需要第二視角 review 時,session 1 可在派工裡明列 Codex CLI review step,交 session 2 執行:
...
適用場景:
- commit 完成後 review 最新 commit diff(`--commit HEAD` = review `HEAD^..HEAD`,不是整 repo / working tree)
- snapshot / handoff / 規則文件這類容易出現內部矛盾的 commit
- Wayne 明確要求「讓 Codex review 看一眼」時

限制:
- `-s danger-full-access -a never` 是全域參數,必須放在 `codex` 後、`review` 前。
- 不用 Claude Code plugin slash command `/codex:review` 當主路徑;Wayne Windows 環境該路徑仍屬半通。
- review finding 是給 Claude 的第二視角 / 導正訊號。派工範圍內明確 actionable finding 可由 session 2 直接修;超出派工範圍 / 牽涉決策才 STOP 給 session 1 / Wayne。
- progress report 避免貼長篇 review 摘要:只寫「無 actionable finding」或「Codex review 指出 X,已修 Y / STOP 原因 Z」。
- Wayne 若說「停止自動出現 Codex review」或同義指令,session 1 立刻停止把 Codex review 自動列入 commit / snapshot / 規則文件派工;之後必須等 Wayne 再次明確呼喊「Codex review」才恢復作動。
...
- 若本批改動包含 snapshot / session handoff / 規則文件 / Codex setup,且 Wayne 未暫停 Codex review 自動出現,可把 `codex -s danger-full-access -a never review --commit HEAD` 列為 commit 後 review step;session 2 可修派工範圍內明確 finding,超出範圍才 STOP。
