...
主視窗草擬 commit message + 派工拆批,執行端跑 `git add` / `git commit -m "..."` / `git push`。push 前 grep `git status` 驗證 staged 對齊,不對 STOP 上報。

紀律源頭在 user-level CLAUDE.md 硬規則 4,本檔同步以便 system-setup repo 自身可讀。SYSADMIN_BRIEFING 規則 11 有完整紀律敘述。

---

**最後更新**:2026-04-29
**版本**:2.1(教訓 12:commit 紀律更新)
