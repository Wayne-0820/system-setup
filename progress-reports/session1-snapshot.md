# session1-snapshot(v19,2026-05-04)
...
(蓋掉 v18 commit `dc78f7b`,2026-05-04;Lane A evidence pack 收尾 commit `898839f` 已落地 origin/main 為 v19 trigger anchor)
...
**待 Wayne 拍下一步**(本檔 v19 已重寫落地 working tree,等 Wayne 拍下方候選之一):

- (a)Batch snapshot v19 single-file commit + push(本檔 v19 single-file commit,對齊歷史 snapshot commit pattern,例 `dc78f7b` v18 / `0b4bf9f` v17 / `98d9cc4` v16 / `cf1a96d` v15)
- (b)Lane A.1 修復試驗派工(`experimental_windows_sandbox` / disable `elevated_windows_sandbox` 試;需備份 `~/.codex/config.toml` + 派工拍板)
- (c)Lane A.2 雙重安裝 conflict probe 派工(`Get-AppxPackageManifest` audit + Lane D1 親跑時 `Get-Process` 看是否啟動 packaged app)
- (d)Lane A.3 接受限制 + 主路徑切換文件化(`/codex:setup` + MCP 為主用法,`/codex:review` 標 known limitation)
- (e)Lane D2 桌面板 GUI 平行用法 probe
...
**本對話新落地(已 commit `898839f` + push origin/main):Lane A evidence pack 整合 + upstream ground truth verify**
...
- Lane A root cause 主嫌排序 → **已更新**(#1 config schema / #2 WindowsApps ACL+雙重安裝 / #3 onboarding 跑了但壞掉)
- Lane A.1 候選修復試驗 → **未做**,Wayne 自決時機(本批硬限制不做)
- Lane A.2 雙重安裝 conflict probe → **未做**,Wayne 自決時機
- Lane A.3 接受限制文件化 → **未做**,Wayne 自決時機(本派工已預列為候選)
...
- Lane A.1 / A.2 / A.3 → 未做,Wayne 自決時機
...
### v19 整合衍生(本批新增,Wayne 拍 — 暫存 reminder,不升級)
...
8. **Codex review handoff pattern(輕量候選)**(v18 既有):SESSION1 寫完 snapshot / 規則 patch / 派工草案後,若 Wayne **顯式指定** Codex review,只交 diff 摘要 + scope + dirty status 給 Codex;Codex 回 `APPROVE` / `TRIM` / `BLOCK`,SESSION1 只依 verdict 修正,不展開新決策包。**不自動 gate 所有輸出**;不寫進 `SESSION_1_MAINWINDOW.md` / 不寫進 `dispatch-commit` skill / 不新增 workflow。**本對話 Lane A patch 落地後 Wayne 顯式 invoke Codex review verdict = APPROVE with one tiny trim(driving down → 降低候選優先度),機制再次驗證可行**

### v18 reminder 延續(觀察候選,本對話無新增 / 無移出)

9. **sanity scan token-prefix regex 缺 word boundary**(v18 Lane A evidence pack progress report §observation #5 延續):派工 §sanity scan token-prefix regex(`sk-[A-Za-z0-9-]+` / `hf_[A-Za-z0-9]+` 等)缺 word boundary `\b...\b`,在含 `disk-` / `risk-` / `task-` 等前綴字串中 false positive 命中。下次 evidence 派工撰寫時可補,**不本輪升級規則**。樣本 1 例延續暫存。

---

## 剩餘 Wayne 拍板項(本對話 Lane A evidence pack 收尾,Lane A.x / D2 / 其他獨立列出)

1. **Lane A.1 修復試驗**(依 `#10090` workaround 試 `experimental_windows_sandbox` / disable `elevated_windows_sandbox`)— 中 — Wayne 自決時機 + 派工拍板
2. **Lane A.2 雙重安裝 conflict probe**(`Get-AppxPackageManifest` audit + Lane D1 親跑時 `Get-Process` 看是否啟動 packaged app process)— 中 — Wayne 自決時機
3. **Lane A.3 接受限制 + 主路徑切換文件化**(`codex/setup.md` patch:`/codex:review` 標 known limitation,主路徑改 `/codex:setup` + MCP)— 低 — Wayne 自決時機
4. **Lane D2:Codex 桌面板 GUI 平行用法實測**(社群證據不足,雙重安裝 evidence 暗示 GUI 走 packaged app 行為可能不同)— 低 — Wayne 自決時機
...
**git HEAD = origin/main = `898839f`**(synced,Lane A evidence pack 收尾 commit)

v19 寫入後 working tree 預期只有本檔 dirty:

```
git status:
  M  progress-reports/session1-snapshot.md  ← 本檔(v19 rewrite,等 Batch commit)
```

下一個 commit(Wayne 若拍 (a))= **Batch snapshot v19**:本檔 single-file commit + push,對齊歷史 snapshot commit pattern(`dc78f7b` v18 / `0b4bf9f` v17 / `98d9cc4` v16 / `cf1a96d` v15)。
...
- **codex 後續 lane**:Lane A.1 / A.2 / A.3 / D2 獨立未拍板,Wayne 自決時機(Lane A evidence pack + B + C + D1 已收尾)
- **下次 cleanup 候選**:本檔 v19 commit + push 落地後,`progress-reports/` 可清舊 raw reports — 候選 9 檔(v18 既有 6 檔 `2026-05-04_commit-snapshot-v16.md` + `2026-05-04_cleanup-raw-reports.md` + `2026-05-04_commit-rules-batch-1.md` + `2026-05-04_commit-snapshot-v17.md` + `2026-05-04_lane-d1-cli-review-probe.md` + `2026-05-04_commit-codex-lane-d1.md`;v19 新增 3 檔 `2026-05-04_commit-snapshot-v18.md` + `2026-05-04_codex-sandbox-evidence-pack.md` + `2026-05-04_commit-codex-lane-a-evidence.md`),保留 README + session1-snapshot;**本派工不做 cleanup**,cleanup 派工由下次主視窗自決時機(避免本對話 scope 漂移)
...
**最優先動作**:Wayne 拍下一步(snapshot v19 commit / Lane A.1/A.2/A.3 / Lane D2 / 其他 / 暫緩收尾)。常見接手場景:

- (a)Wayne 想直接拍 snapshot v19 commit 派工:主視窗草擬本檔 single-file commit + push 派工
- (b)Wayne 暫緩 commit,只想 audit / 整合 / 對話:不需動派工(working tree 留 snapshot.md dirty handoff 到下次接手)
- (c)Wayne 想拍其他 lane(A.1 / A.2 / A.3 / D2 / WAN2.2 / dogfood / 雙 session ROI / cleanup):走規劃 + 派工流程
...
- **Codex 整合** → `codex/setup.md`(三件套配置 + sandbox / mirror / keyword scan 踩坑 + mirror 紀律 + Lane A/D + 本對話 Lane A evidence pack 整合 + upstream verify 對照)+ root `AGENTS.md`(hand-crafted Codex source-of-truth,Lane C 落地)
...
**蓋掉**:v18 commit `dc78f7b`(2026-05-04)
**v19 更新理由**:本對話 Lane A evidence pack 收尾 commit `898839f` 落地 origin/main(`codex/setup.md` 三段更新 + 1 處 trim:坑 #1 §平行用法 cross-cut + 坑 #1 §forensic 觀察 + Lane A 整段重寫 + Codex review trim,共 35 insertions / 8 deletions)。Lane A evidence pack(read-only)結果 + 4 個 upstream issue 主視窗 ground truth verify 整合落地,根因排序更新(#1 `elevated_windows_sandbox` config schema 主嫌 / #2 WindowsApps packaged app ACL+雙重安裝 / #3 onboarding 跑了但壞掉)。Lane A.1 修復試驗 / Lane A.2 雙重安裝 probe / Lane A.3 接受限制文件化 列為 Wayne 自決時機候選。Codex 三件套 lane 累積:Lane B + C + D1 + A evidence pack 已收尾 / Lane D2 + A.1 + A.2 + A.3 仍 Wayne 自決時機。v18 整合衍生 t2 / t3 已由 `898839f` cover 從暫存移出;v17/v18 整合衍生 #14(commit `-F file` 路線)累積樣本上升至 5 次,**列升級評估候選但不本輪升級**;v18 整合衍生 #15(Codex review handoff)本對話再次驗證可行;v19 新增整合衍生 #17(派工 grep verify 合法 reference false STOP),樣本 1 例暫存不升 SOP。剩 8 條等拍項(Lane A.1 / A.2 / A.3 / D2 / WAN2.2 / dogfood / 雙 session ROI / `-F file` 升級時機延續)。
